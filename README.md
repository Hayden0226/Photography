...
<details open>
<summary><strong>v0.4.4</strong> · 管理模式新增删除照片 · 网络与重命名一致性加固</summary>

- **删除照片**：管理模式新增红色「删除该照片」操作，二次确认后从私人照片仓库永久删除照片，并自动清理主仓库 `content/photo-descriptions.json` 的对应条目；删除不可撤销，旧链接随之失效。
- **修复删除中断残留**：删除分「删除照片文件 → 清理描述条目」两步，中途网络失败会留下孤儿描述条目；再次删除时若照片文件已不存在，会自动走残留清理路径（只清理描述），不再误报失败。
- **修复确认执行无反应**：面板所有管理操作整体捕获异常，GitHub API 或网络错误会在结果区回显明确信息，不再出现点击「确认执行」无响应。
- **GitHub API 网络自动重试**：瞬时网络故障（连接被拒、请求中断等）自动重试最多 3 次（400ms 指数退避）；HTTP 错误状态（404/409 等）不重试并保留具体报错。
- **重命名同步标题**：重命名照片文件后自动把描述条目中的标题（`zh-CN`/`en`）同步为新文件名，杜绝主页悬停提示仍显示旧标题的问题。
- **基本信息显示真实文件名**：照片详情「基本信息」中的文件名改为显示真实文件名，不再误显示照片标题。
