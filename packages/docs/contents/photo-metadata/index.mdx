...
Jacky's Photography keeps generated camera metadata in the builder manifest and keeps human-written copy in `content/photo-descriptions.json`. The sidecar file is merged into the manifest during `pnpm run build:manifest`, so editorial descriptions can improve search, alt text, detail pages, and social sharing without hand-editing generated manifest output.
...
This site intentionally publishes the exact GPS coordinates stored in photo EXIF. Production builds emit those coordinates in the public, content-hashed full manifest, and the map downloads that manifest to place photos at their precise locations. Coordinates are not blurred or rounded; this is a deliberate publication policy for Jacky's Photography.
