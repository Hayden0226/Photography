...
This site intentionally publishes the exact GPS coordinates stored in photo EXIF. Production builds emit those coordinates in the public, content-hashed full manifest, and the map downloads that manifest to place photos at their precise locations. Coordinates are not blurred or rounded; this is a deliberate publication policy for Hayden's Photography.
...
Deployers who do not accept that policy must add a builder-stage sanitizer that removes the complete GPS field set before the manifest is saved and before the web build runs. Disabling or hiding the map is not sufficient because the full manifest remains a directly downloadable public asset. After sanitizing, verify both the generated source manifest and the emitted `assets/photos-manifest.<hash>.json` before deployment.
