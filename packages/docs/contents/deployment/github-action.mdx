---
title: GitHub Action
description: Current GitHub Actions validation and deployment workflow for Hayden's Photography.
...
- `workflow_dispatch`
- `repository_dispatch` with type `photos-updated`, sent by the private photo repository
...
9. Checks the bundle budget and verifies `apps/web/dist/`.
10. Installs Playwright Chromium and runs `pnpm run test:e2e` with `PLAYWRIGHT_PRODUCTION=true`.

PR validation never checks out the private photo repository, reads deployment secrets, pushes photo changes, syncs R2, or mirrors deployment output.

## Production Validation

The `build-and-deploy` job first verifies that every required deployment secret is present. It then checks out the private photo repository into `./photos`, rejects every symlink outside the nested `.git` directory, and runs the real-photo pipeline:
...
8. Run the production-mode Playwright suite.
...
1. Commits standardized photo changes back to the private photo repository with `[skip dispatch]`.
2. Syncs `./photos` to Cloudflare R2 under the `photos/` prefix.
3. Checks out the deployment repository and mirrors `apps/web/dist/` into it.
...
- `PHOTO_REPO_TOKEN`: fine-grained PAT with Contents access to the private photo repository.
- `DEPLOY_REPO_TOKEN`: token used to push static output to the deployment repository.
...
The private photo repository requires `MAIN_REPO_DISPATCH_TOKEN`, a token that can send `repository_dispatch` events to the source repository.
...
- `https://<你的图片域名>/photos/` is the public URL used in the manifest.
...
Photo source files are not tracked in this public repository. The public site loads original photos from `https://<你的图片域名>/photos/`, backed by Cloudflare R2.
