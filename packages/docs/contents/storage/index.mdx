...
description: Current storage-provider setup and supported alternatives for Hayden's Photography.
...
`@afilmory/builder` reads photos through a storage-provider abstraction. Hayden's Photography currently uses the local provider against a private photo-repository checkout during builds, then syncs the published photo tree to Cloudflare R2 for public delivery.
...
In local development, `./photos` is a checkout of the private photo repository. Production GitHub Actions runs check out the same private repository into `./photos`, reject symlinks, standardize incoming files, build the manifest in strict mode, and sync published files to Cloudflare R2.

Pull-request validation does not access the private repository. It generates a public synthetic photo fixture, points the local provider at that fixture through `AFILMORY_PHOTOS_PATH` and `AFILMORY_PHOTOS_BASE_URL`, and exercises the same strict manifest and production E2E paths without photo or deployment secrets.
