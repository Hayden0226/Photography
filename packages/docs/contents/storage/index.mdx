...
Jacky's Photography currently uses the local provider against a private photo-repository checkout:
...
`excludeRegex` keeps staging files under `photos/incoming/` out of the published manifest until they have been standardized or moved into a real category folder. In CI, `./photos` is checked out from `Jackyhq/Photography-Photos`, and published files are synced to Cloudflare R2 behind `https://photos3.jackyw.cn/photos/`.

The production R2 layout uses a bucket plus a prefix rather than a separate bucket for photos. For example, the workflow sync target is `s3://<bucket>/photos/`, while `builder.config.ts` publishes URLs with the public base URL `https://photos3.jackyw.cn/photos/`. Keep the GitHub secret `CLOUDFLARE_R2_BUCKET` set to the bucket name only; the `photos/` folder is provided by the workflow prefix.
...
5. **Thumbhash Placeholder Generation** - Creates compact placeholder data for smooth loading
