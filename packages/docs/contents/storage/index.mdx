...
In local development, `./photos` is a checkout of `Jackyhq/Photography-Photos`. Production GitHub Actions runs check out the same private repository into `./photos`, reject symlinks, standardize incoming files, build the manifest in strict mode, and sync published files to Cloudflare R2.
