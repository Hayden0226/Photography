...
The documentation site is a Vite, React, and MDX static app in `packages/docs`. It documents this customized Jacky's Photography repository, not upstream Afilmory in general. Keep examples aligned with the current workspace, deployment workflow, private photo checkout, and `builder.config.ts`.
