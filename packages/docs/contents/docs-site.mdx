---
title: Docs site
description: How to maintain and publish the Hayden's Photography documentation site at docs.visuals.haydenweb.com.
...
The documentation site is a Vite, React, and MDX static app in `packages/docs`. It is published at the docs site and documents this customized Hayden's Photography repository, not upstream Afilmory in general. Keep examples aligned with the current workspace, deployment workflow, private photo checkout, and `builder.config.ts`.

## Public URLs

- Documentation site: the docs site
...
- `index.mdx` introduces the project, workspace, manifest flow, and deployment flow.
...
`pnpm docs:build` runs the package build pipeline: route generation, TypeScript and Vite client build, SSR/static rendering, table-of-contents extraction, and final output processing through `packages/docs/scripts/build.ts`.
...
- Document `apps/web/dist/` as the web build output and the deployment repository as the mirrored deployment repository.
